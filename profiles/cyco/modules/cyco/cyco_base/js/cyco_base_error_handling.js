...
(function ($) {
  Drupal.behaviors.cyCoErrorHandler = {
    attach: function() {
      //Set namespace ref.
      Drupal.behaviors.cyCoErrorHandler = Drupal.behaviors.cyCoErrorHandler;
    }, //End attach
    reportError: function( message ) {
      //Show a "Please wait" display.
      Drupal.behaviors.cyCoErrorHandler.showAjaxWaitDisplay( message );
      $.when(
        //Send the error report to the server.
        Drupal.behaviors.cyCoErrorHandler.sendErrorToServer( message )
      )
      .then(function() {
        //Let the user hide the "Please wait" display.
        Drupal.behaviors.cyCoErrorHandler.activateCloseButton();
      })
      .fail(function(jqXHR, textStatus, errorThrown) {
        //Error while reporting an error.
        alert( Drupal.behaviors.cyCoErrorHandler.showMetaError( errorThrown ) );
      });
    }, //End reportError
    ...
    showAjaxWaitDisplay: function( message ) {
      ...
      if ( $("#cyco-client-error-container").length == 0 ) {
        ...
        $("#cyco-client-error-button").click(function() {
          Drupal.behaviors.cyCoErrorHandler.hideAjaxWaitDisplay()
        });
      }
